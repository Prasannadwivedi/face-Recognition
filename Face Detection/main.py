...
def find_matching_id(id_dataset, embedding):
    ...
    return matching_id, min_dist                                        #in terminal


def get_embedding_distance(emb1, emb2):
    dist = np.sqrt(np.sum(np.square(np.subtract(emb1, emb2))))          #distance calculation
    return dist                                                         #return distance to variable dist
...
def test_run(pnet, rnet, onet, sess, images_placeholder, phase_train_placeholder, embeddings, id_dataset, test_folder):
    ...
    image_names = os.listdir(os.path.expanduser(test_folder))                   # RETURNS NAME OF IMAGES IN THE FOLDER 
    image_paths = [os.path.join(test_folder, img) for img in image_names]       # LIST OF PATHS OF IMAGES IN TEST_FOLDER
    nrof_images = len(image_names)                                              # NUMBER OF IMAGES IN THE FOLDER
    aligned_images = []
    aligned_image_paths = []
